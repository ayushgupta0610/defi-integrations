...
import UniswapV2Wrapper from "../artifacts/contracts/UniswapV2Wrapper.sol/UniswapV2Wrapper.json";
...
function App() {
  ...
  async function getGasPrice(txnSpeed = "standard") {
    try {
      ...
      const maxPriorityFeePerGas =
        Math.ceil(data[txnSpeed].maxPriorityFee) * 10 ** 9;
  ...
  async function checkLiquidity(token1, token2) {
    try {
      ...
      const uniswapV2Wrapper = new ethers.Contract(
        config[network].exampleDappAddress,
        UniswapV2Wrapper.abi,
        signer
      );
      ...
      const pairInfo = await uniswapV2Wrapper.pairInfo(token1, token2);
      console.log(
        `USDC: ${token1} reserve is: ${pairInfo.reserveA.toString()}`
      );
      console.log(
        `MATIC: ${token2} reserve is: ${pairInfo.reserveB.toString()}`
      );
  ...
  async function getLiquidtyPairAddress(token1, token2) {
    try {
      ...
      console.log(
        `Pair address for ${token1} and ${token2} is: ${pairAddress}`
      );
    } catch (error) {
      console.error(error);
    }
  ...
  async function swapToMatic(token1) {
    try {
      ...
      const allowanceCheck = await tokenInstance.allowance(
        walletAddress,
        config[network].exampleDappAddress
      );
      ...
      const swapForETH = await uniswapV2Wrapper.swapForETH(
        token1,
        token1Value,
        amountOutMin,
        { maxFeePerGas, maxPriorityFeePerGas }
      ...
      console.log(
        `Swap for ETH txn confirmed: ${swapForETHReceipt.transactionHash}`
      );
    } catch (error) {
      console.error(error);
    }
